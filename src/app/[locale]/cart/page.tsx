...
export default function CartPage() {
  ...
  const checkout = async () => {
    ...
    if (saleRes.ok) {
      ...
      setCheckoutError(err.error ?? 'Error al procesar la compra');
    }
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-3">
              {cartItems.map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`flex gap-4 bg-[#fefcf5] border-2 p-4 ${outOfStock ? 'border-red-600' : 'border-[#1c1410]'}`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="font-black text-[#1c1410] uppercase text-xs tracking-wide line-clamp-2">
                        {item.productId.name}
                      </h3>
                      <p className="text-[#5c3d2e] text-xs mt-1 uppercase tracking-wide">
                        {t('quantity')}: {item.quantity}
                      </p>
                      <p className="font-bold text-[#1c1410] mt-1 text-sm">
                    ...
                    <button
                      ...
                      className="text-xs font-bold uppercase tracking-wide text-[#5c3d2e] hover:text-red-700 transition-colors self-start flex-shrink-0"
                    >
                      {t('remove')}
                    </button>
                  </div>
            ...
            <div className="bg-[#fefcf5] border-2 border-[#1c1410] p-6 h-fit">
              <h2 className="text-xs font-black uppercase tracking-widest text-[#1c1410] mb-4 border-b-2 border-[#1c1410] pb-2">
                Resumen
              </h2>
              ...
              <div className="space-y-2 mb-4">
                {cartItems.map((item, idx) => (
                  <div key={idx} className="flex justify-between text-xs text-[#5c3d2e]">
                    ...
                    <span className="font-bold text-[#1c1410] ml-2">
                      $ {(item.productId.price * item.quantity).toFixed(2)}
                    </span>
                  </div>
                ))}
              ...
              {checkoutError && (
                <p className="text-xs font-bold text-red-700 bg-red-50 border-2 border-red-600 px-3 py-2 uppercase tracking-wide mb-4">
                  {checkoutError}
                </p>
              )}

              {hasOutOfStock && (
                <p className="text-xs font-bold text-red-600 border-2 border-red-600 px-3 py-2 uppercase tracking-wide mb-4">
                  ⚠ Retira los artículos sin stock para continuar
                </p>
              )}
